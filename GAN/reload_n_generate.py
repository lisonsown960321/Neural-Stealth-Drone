...
for epoch in range(num_epochs):
    ...
    for i, (data, labels) in enumerate(dataloader, 0):
        ...
        if i != None:
            ...
            temlist = list_float(store_temp.tolist())
            for kk in range(0,len(temlist)):
                temlist[kk].append(1.11)
            fake_feature_maps = fake_feature_maps + temlist
...
for i, (data, labels) in enumerate(dronedesignloader, 0):
    ...
    if i != None:
        ...
        for kk in range(0,len(temlist)):
            temlist[kk].append(labels[kk])
        design_feature_maps = design_feature_maps + temlist
...
with open('/Users/lichen/Desktop/test/WGAN_generated/design_feature_maps.csv', 'w') as myfile:
    wr = csv.writer(myfile)
    wr.writerows(design_feature_maps)